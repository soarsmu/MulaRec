...
class APIDataset(data.Dataset):
    ...
    """
    def __init__(self, desc_file, api_file, max_seq_len=50, context_api_file=None):
        ...
        if context_api_file:
            self.is_test=True
            context_api_table = tables.open_file(context_api_file)
            self.context_api_data = context_api_table.get_node('/phrases')[:].astype(np.long)
            self.context_api_index = context_api_table.get_node('/indices')[:]
    ...
    def __getitem__(self, offset):
        desc_len = self.max_seq_len-2
        
        # try:
        if 0 in self.desc_data[offset]:
            desc_len = min(self.desc_data[offset].tolist().index(0), self.max_seq_len-2)
        # except:
            # print(f'self.desc_data[offset]: {self.desc_data[offset]}')
        
        data = self.desc_data[offset][:desc_len].tolist()
        desc = [SOS_ID] + data + [EOS_ID]

        api_len = self.max_seq_len-2
        if 0 in self.api_data[offset]:
            api_len = min(self.api_data[offset].tolist().index(0), self.max_seq_len-2)

        data = self.api_data[offset][:api_len].tolist()
        api = [SOS_ID] + data + [EOS_ID]
        ...
        if self.is_test:
            context_api_len = self.max_seq_len-2
            if 0 in self.context_api_data[offset]:
                context_api_len = min(self.context_api_data[offset].tolist().index(0), self.max_seq_len-2)
            context_api = self.context_api_data[offset][:context_api_len].tolist()
            context_api = self.list2array(context_api, self.max_seq_len, np.long, PAD_ID)
            return desc, desc_len, api, api_len, context_api, context_api_len
        return desc, desc_len, api, api_len
