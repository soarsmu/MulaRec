...
class RNNDecoder(nn.Module):
    ...
    def beam_decode(self, init_h, enc_hids, src_pad_mask, context, beam_width, max_unroll, topk=1, context_apis=None, context_api_lens=None):
        '''
        ...
        for idx in range(batch_size): # decoding goes sentence by sentence
            if isinstance(init_h, tuple):  # LSTM case
                ...
                if context_apis is not None:
                    if len(context_apis[idx:idx+1]) > 0:
                        context_api = context_apis[idx:idx+1][0][:context_api_lens[idx:idx+1][0]]

            print(f"context_api: {context_api}")
            print(f"len context_api: {len(context_api)}")

            context_api_length = len(context_api)
            ...
            counter = 1
            node = BeamNode(h, None, x, 0, counter)
            nodes = PriorityQueue()

            # start the queue
            # need to put context api here
            ...
            while True:                
                ...
                # print(f"best score node: {score}")
                x = n.wordid
                # print(f"best word id: {x}")
                h = n.h
                qsize-=1
                ...
                if len(context_api) < 1:
                    ...
                    i = 0
                    for word_idx in context_api:
                        node = BeamNode(h, n, torch.LongTensor(1,1).fill_(word_idx), math.log(i+1), n.len + 1)
                        score = -node.eval()
                        i += 1
            ...
            for score, n in sorted(endnodes, key=operator.itemgetter(0)):
                ...
                # remove context_api
                utterance = utterance[context_api_length:]
                length = length - context_api_length
