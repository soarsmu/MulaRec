...
def evaluate(model, metrics, test_loader, vocab_desc, vocab_api, repeat, decode_mode, f_eval, f_eval_single=None, is_test=False):
    ...
    if is_test:
        for descs, desc_lens, apiseqs, api_lens, context_apis, context_api_lens in tqdm(test_loader):
            ...
            with torch.no_grad():
                sample_words, sample_lens = model.sample(descs, desc_lens, repeat, decode_mode=decode_mode, context_apis=context_apis, context_api_lens=context_api_lens)
            ...
            for r_id, pred_sent in enumerate(pred_sents):
                ...
                if r_id == 0:
                    f_eval_single.write(pred_sent.replace(" ' ", "'"))
                    f_eval_single.write("\n")
            f_eval.write("\n")
...
def main(args):
    ...
    # Load data
    test_set=APIDataset(args.data_path+'test.desc.h5', args.data_path+'test.apiseq.h5', conf['max_sent_len'], context_api_file=args.data_path+'test.context_api.h5')
    ...
    f_eval = open(f"./output/{args.model}/{args.expname}/results_211022.txt".format(args.model, args.expname), "w")
    f_eval_single = open(f"./output/{args.model}/{args.expname}/results_single_211022.txt".format(args.model, args.expname), "w")
    
    evaluate(model, metrics, test_loader, vocab_desc, vocab_api, args.n_samples, args.decode_mode , f_eval, f_eval_single=f_eval_single, is_test=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--expname', type=str, default='141022-3lines', help='experiment name, disinguishing different parameter settings')
    parser.add_argument('--timestamp', type=str, default='202210161126', help='time stamp')
    parser.add_argument('--reload_from', type=int, default=95000, help='directory to load models from')
    
    parser.add_argument('--n_samples', type=int, default=10, help='Number of responses to sampling')
    parser.add_argument('--decode_mode', type=str, default='beamsearch',
